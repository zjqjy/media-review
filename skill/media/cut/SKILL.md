...
4. **重录对（口误→停顿→重说）不由初选拍板删**：转录已自动检测（相邻条目归一化相似度≥0.8 + 停顿间隔），并按用户规则**保最后一个、前遍进词级洞挖掉**；标注轨同时保留"🔁重录对"提示，用户听音后想换回前遍，把剪切稿里末遍条删掉、前遍条加回即可。纯文本判断不了哪遍顺，AI 不越权
5. 产出两个文件：
