...
2. 读转录稿逐句标三类：
   - **保留**：核心步骤、结论、金句
   - **删**：跑题、纯语气词条目、语义重复段（用户拍板 2026-09-19：**意思重复=默认重录，删前留后**——哪怕两遍措辞不同，也只留最后那遍；用户反馈"出现重复意思的段落都是在重录"，逐句人工比对费时间不值得）
...
4. **逐字重录对（口误→停顿→重说）由转录自动处理**：检测后按用户规则保最后一个、前遍进词级洞挖掉；标注轨保留提示供听音推翻。**语义级重复**（措辞不同意思一样，自动检测抓不全）由初选拍板：同样**删前留后**，删前在复核单列组说明（"这段你重复讲了 X，留了最后一遍"），用户不同意再捞回
5. 产出两个文件：
   - **剪切稿**：删条目后的 srt（用户复核后本 skill 代跑 `python scripts/srt_cut.py 素材_trimmed.mp4 剪切稿.srt` 出粗片）
   - **复核单**：删了哪些句 + 为什么，拿不准的标"建议保留"让用户定；重录对单列一组（保了哪遍、两遍原文对照）
6. 初选时用认知②打分：优先保住"清单感"（步骤完整、结论前置、每步可跳读），别为完播砍步骤

**流程定调（2026-09-19 用户拍板：一步到位，不再人工复核初选）**——AI 初选直接产出最终剪切稿并生成剪映草稿，复核单只作附录供事后翻查，不是门禁。删错的句子用户在剪映里拖回来（草稿段都是可拖分割线），比事前逐句确认快。
...
## 工作区规矩（2026-09-19 实测沉淀）

- **跑完管线执行 `organize.py <待粗剪目录>`** 归位阶段目录（1_原始/2_静音压缩/3_转录初选/_工具），不靠人肉整理；被占用的文件跳过并报告，关程序后重跑
- **修错表应用前先 dry-run**：将替换的原/改句子对照打印过目再落盘（SSP三二→SESP32 返工教训）
- **跨文件重复不做自动检测**：措辞不同的同场景重拍纯文本抓不到（实测 sim 仅 0.32），人工对读兜底
- **零重编码架构**（roadmap）：①层目前物理重编码 trimmed.mp4（17 分钟大头），对③层冗余——jy_draft 引用原素材且自带静音细剪。改造方向：①层只输出保留区间清单，转录吃原视频音频+重映射，大文件编码移出关键路径

## 红线

- 只处理**用户自己的素材**，不给脚本加任何爬取/下载他人视频的能力
- AI 初选一步到位直接出剪映草稿（2026-09-19 用户决策）；品味判断后移到精剪
